...
def panel(surf: pygame.Surface, rect: pygame.Rect, fill=PANEL_FILL,
    ...
    pygame.draw.rect(tmp, fill, rect, border_radius=radius)
    pygame.draw.rect(tmp, border, rect, width=1, border_radius=radius)
    surf.blit(tmp, (0, 0))
...
class Button:
    ...
    def draw(self, surf: pygame.Surface, fonts: Fonts) -> None:
        ...
        y = self.rect.centery - (8 if self.sub else 0)
        text(surf, fonts, self.label, 20 if self.sub else 22, (self.rect.centerx, y),
             ...
             anchor="center")
        if self.sub:
            text(surf, fonts, self.sub, 14, (self.rect.centerx, y + 26),
...
def draw_preview(surf: pygame.Surface, fonts: Fonts, title: str,
                 ...
                 cell: int = 16, skin: dict | None = None) -> None:
    """在方框内用迷你等距方块绘制预览（外观跟随当前方块皮肤）"""
    text(surf, fonts, title, 16, (center[0], center[1] - 80),
         color=TEXT_DIM, anchor="center", shadow=False)
    panel(surf, pygame.Rect(center[0] - 52, center[1] - 58, 104, 116), radius=10)
    if not piece_name:
        return
    from .pieces import ALL_CELLS
    from .renderer import iso, skin_face_colors
    from .constants import SKINS
    skin = skin or SKINS["neon"]
    color = PIECE_C(piece_name)
    cells = ALL_CELLS[piece_name][0]
    # 整体居中
    pts = [iso(c, r) for c, r in cells]
    ...
    for (c, r) in cells:
        x0, y0 = ox + iso(c, r)[0], oy + iso(c, r)[1]
        x1, y1 = ox + iso(c + 1, r)[0], oy + iso(c + 1, r)[1]
        x2, y2 = ox + iso(c + 1, r + 1)[0], oy + iso(c + 1, r + 1)[1]
        x3, y3 = ox + iso(c, r + 1)[0], oy + iso(c, r + 1)[1]
        t = cell  # 立方体高度
